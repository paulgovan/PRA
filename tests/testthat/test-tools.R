...
test_that("risk_prob_tool computes correct prior probability", {
  ...
  text <- tool_text(result)
  # Noisy-OR of marginals 0.38 and 0.44: 1 - (1-0.38)(1-0.44) = 0.6528
  expect_true(grepl("0\\.6528", text))
  expect_true(grepl("65\\.28", text)) # 65.28%
})
...
test_that("risk_post_prob_tool computes correct posterior probability", {
  ...
  text <- tool_text(result)
  # Cause 1 observed present -> 0.8; Cause 2 unknown -> marginal 0.44.
  # Noisy-OR: 1 - (1-0.8)(1-0.44) = 0.888. Observing an adverse cause raises it.
  expect_true(grepl("0\\.888", text))
  expect_true(grepl("88\\.8", text)) # 88.8%
...
test_that("risk_post_prob_tool with all causes observed", {
  ...
  text <- tool_text(result)
  # Cause 1 present -> 0.8; Cause 2 absent -> 0.4.
  # Noisy-OR: 1 - (1-0.8)(1-0.4) = 0.88
  expect_true(grepl("0\\.88", text))
  expect_true(grepl("Occurred", text))
