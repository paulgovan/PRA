...
PRA can expose all its analytical tools as an
[MCP (Model Context Protocol)](https://modelcontextprotocol.io) server via the
[mcptools](https://posit-dev.github.io/mcptools/) package. This lets Claude
Desktop, Claude Code, or any MCP-compatible AI client call PRA functions
directly — no R session or Shiny app required on the user's end.
...
`pra_tools()`: MCS, SMM, EVM, contingency, sensitivity, Bayesian risk,
learning curves, and DSM analysis.
...
By default, each `Rscript` invocation starts a fresh R session. To preserve
simulation results across tool calls within a conversation (e.g., run MCS
then immediately calculate contingency on those results), use
`mcptools::mcp_session()` in an interactive R session before connecting:
