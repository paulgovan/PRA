...
|----------|--------------|---------|---------|
| `mcs(num_sims*, task_dists*, cor_mat)` | `num_sims` number of simulations; `task_dists` list of distribution specs; optional correlation matrix | S3 object of class `mcs` with `$simulations` matrix, `$total_distribution`, percentiles | @sec-mcs |
...
| Function | Key Arguments | Returns | Chapter |
|----------|--------------|---------|---------|
| `smm(mean*, var*, cor_mat)` | Numeric vectors of task means and variances; optional correlation matrix | S3 object of class `smm` with `$total_mean`, `$total_var`, `$total_sd` | @sec-smm |
...
| Function | Key Arguments | Returns | Chapter |
|----------|--------------|---------|---------|
| `pv(bac*, schedule*, time_period*)` | Budget at completion; cumulative schedule proportions; current period (1-based) | Planned Value | @sec-evm |
| `ev(bac*, actual_per_complete*)` | Budget at completion; fraction complete (0–1) | Earned Value | @sec-evm |
| `ac(actual_costs*, time_period*, cumulative)` | Vector of actual costs per period; current period; whether `actual_costs` is already cumulative (default `TRUE`) | Actual Cost | @sec-evm |
...
| `eac(bac*, method, cpi, ac, ev, spi)` | BAC; `method` = "typical" / "atypical" / "combined" (default "typical"); `cpi` required for typical; `ac`/`ev` required for atypical; `cpi`/`ac`/`ev`/`spi` required for combined | Estimate at Completion | @sec-evm |
| `etc(bac*, ev*, cpi)` | Budget at Completion; Earned Value; Cost Performance Index | Estimate to Complete | @sec-evm |
| `tcpi(bac*, ev*, ac*, target, eac)` | BAC, EV, AC; `target` = "bac" or "eac" (default "bac"); `eac` required when `target = "eac"` | To-Complete Performance Index | @sec-evm |
...
| Function | Key Arguments | Returns | Chapter |
|----------|--------------|---------|---------|
| `fit_sigmoidal(data*, x_col*, y_col*, model_type*)` | `data` a data frame; `x_col`/`y_col` column names as strings; `model_type` = "logistic" / "gompertz" / "pearl" | Fitted `nls` model object | @sec-sigmoidal |
| `predict_sigmoidal(fit*, x_range*, model_type*, conf_level)` | Object from `fit_sigmoidal()`; new x values; model type; optional confidence level | Data frame of predictions (with confidence bounds if `conf_level` set) | @sec-sigmoidal |
| `plot_sigmoidal(fit*, data*, x_col*, y_col*, model_type*, conf_level)` | Object from `fit_sigmoidal()`; original data frame and column names; model type; optional confidence level | Plot of fitted curve with confidence band | @sec-sigmoidal |
...
| Function | Key Arguments | Returns | Chapter |
|----------|--------------|---------|---------|
| `parent_dsm(S*)` | Resource-Task matrix S (resources × tasks, binary) | S3 object of class `dsm` (`$type` is `"parent"`); `$matrix` is tasks × tasks | @sec-dsm |
| `grandparent_dsm(S*, R*)` | Resource-Task matrix S; Risk-Resource matrix R (risks × resources, binary) | S3 object of class `dsm` (`$type` is `"grandparent"`); `$matrix` is tasks × tasks | @sec-dsm |
...
| Function | Key Arguments | Returns | Chapter |
|----------|--------------|---------|---------|
| `pra_tools()` | None | List of `ellmer` tool definitions wrapping the PRA functions | @sec-agent |
