...
task_dists <- list(
  list(type = "normal",     mean = 30, sd = 4),
  list(type = "triangular", a = 12, b = 20, c = 30),
  list(type = "uniform",    min = 10, max = 25)
)
...
Every code block you have read throughout this book, including the `mcs()` call in @sec-mcs, the `evm` calculations in @sec-evm, and the Bayesian updates in @sec-bayes, was executed when the book was built. There is no "trust us, this is what you'd see." The numbers are real, freshly computed from the current version of the `PRA` package.
