...
The output is a named vector — one sensitivity index per task. A higher value means that task's variance has a larger proportional effect on total project variance.
...
Expressing the indices as percentages makes the picture clear: Task B (triangular) carries the largest share of total project variance. This is the task where mitigation effort pays off most.
...
Real projects have correlated tasks — they share resources, face the same weather, or depend on the same supplier. Positive correlation amplifies the dominant driver; it becomes even more important to get that task right.
...
cm <- cor_matrix(
  ...
  num_vars    = 3,
  dists = list(
    TaskA = function(n) rnorm(n, 10, 2),
    TaskB = function(n) {
      mc2d::rtriang(n, min = 5, mode = 10, max = 15)
    },
    TaskC = function(n) runif(n, 8, 12)
  )
)
cm
```
...
```{r}
#| fig-cap: "Sensitivity indices when tasks are correlated. Positive correlations amplify the dominant driver (Task B) and increase total project variance."
