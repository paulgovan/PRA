...
Some chapters (especially the network chapters) use additional packages like `igraph` and `networkD3`, and the agentic chapter uses `ellmer`, `mcptools`, and `jsonlite` for the MCP server. Prerequisites are listed at the top of each chapter.
...
> Govan, P. (2025). *Project Risk Analysis: A Practical Guide: From Monte Carlo to AI Agents*. <https://github.com/paulgovan/PRA>
...
The `PRA` package and this book grew out of research on the resource-based view of project risk management [@govan2016; @govan2014]. Thanks to the open-source R community for the packages that make quantitative risk analysis accessible, and to every project manager who has ever looked at a schedule baseline and wondered whether the numbers were as solid as they looked.
